...
final class SessionManager: NSObject, WCSessionDelegate {
    @MainActor static let shared = SessionManager()
    var receivedSessions: [SkateSession] = []
    ...
    nonisolated func session(_ session: WCSession, didReceiveMessage message: [String : Any]) {
        guard let sessionData = message["session"] as? Data else { return }
        Task { @MainActor in
            do {
                let receivedSession = try JSONDecoder().decode(SkateSession.self, from: sessionData)
